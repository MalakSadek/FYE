...
public class ClubsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!isConnected(ClubsActivity.this)) {
            buildDialog(ClubsActivity.this).show();
            return;
        }
            else {
    ...
    public void findStores(){
        String url = "http://188.226.144.157/group1/find_allclubs.php";
        if (requestQueue == null)
        requestQueue = Volley.newRequestQueue(this);

        jsonArrRequest = new JsonArrayRequest(Request.Method.POST, url, jsonArr, new Response.Listener<JSONArray>() {
            @Override
    ...
    private void fillList(final JSONArray resp,final int i) throws JSONException {
        indexMap.add(i);
        String url = "http://188.226.144.157/group1/find_clubs_data.php";
        if (requestQueue == null)
        requestQueue = Volley.newRequestQueue(this);
