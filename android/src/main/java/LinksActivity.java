...
public class LinksActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!isConnected(LinksActivity.this)) {
            buildDialog(LinksActivity.this).show();
            return;
        }
        else {
        ...
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(urls[list.getCount()-position-1]));
    ...
    public void findStores(){
        String url = "http://188.226.144.157/group1/find_links_names.php";
            ...
            public void onResponse(JSONArray response1) {
                try {
                    for (int i = 0;i<response1.length();i++) {
                        final int finalI = i;
                        Log.i("YARAB",response1.getString(i));
    ...
    private void fillList(final JSONArray resp,final int i) throws JSONException {
        indexMap.add(i);
        String url = "http://188.226.144.157/group1/find_links_data.php";
        if (requestQueue == null)
        ...
        StringRequest Stringrequest = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    Log.i("bbb",tmp.name);
                    urls[indexMap.get(k)] = storeData.getString("link");
                    tmp.logo = "link.png";
                    links.add(0, tmp);
                    ListViewAdapter CA = new ListViewAdapter(getApplicationContext(), links);
                    list.setAdapter(CA);
                    k = k+1;
                } catch (JSONException e) {
                    e.printStackTrace();
